<template>
  ...
  <v-dialog max-width="500">
      <template v-slot:activator="{ props: activatorProps }">
        <v-list-item>
        <v-list-item-title v-bind="activatorProps" style="font-family:arial; color: black;">Recent Domains</v-list-item-title>
      </v-list-item>
      </template>
  
      <template v-slot:default="{ isActive }">
          <v-card>
            <v-toolbar color="primary" title="Recent Domains" STYLE="font-size:22.0pt;font-family:arial; color: white;"></v-toolbar>
              <v-card-text class="compact-form">
                <recentDomains @click="closeEdit(isActive, false)" />
              </v-card-text>
