<template>
  <v-col cols="4" align="right">
    <v-row >
    <div align="right" style="width: 100%;padding-top:12px;background-color: white;" >  
      <search v-if="xLocation =='index'" />  
      <v-btn v-if="xLocation !='index'" density="comfortable" rounded="sm" color="iconic" variant="outlined" icon="mdi-domain" nuxt to="/"><v-icon color="iconic">mdi-domain</v-icon><v-tooltip class="sitesdesc" activator="parent" location="start">Explore Domains</v-tooltip></v-btn>
      <span v-if="xLoginStatus == 1" STYLE="font-size:22.0pt;font-family:arial; color: white;">-</span>
      <edit v-if="xLocation == 'home' && xLoginStatus == 1" />  
      <v-btn v-if="xLocation !='home' && xLoginStatus == 1" density="comfortable" rounded="sm" color="iconic" variant="outlined" icon="mdi-home" nuxt to="/home"><v-icon color="iconic">mdi-home</v-icon><v-tooltip class="sitesdesc" activator="parent" location="start">Home Domain</v-tooltip></v-btn>
      <span STYLE="font-size:22.0pt;font-family:arial; color: white;">-</span>
      <edit v-if="xLocation =='local'" />  
      <v-btn v-if="xLocation !='local'" density="comfortable" rounded="sm" color="iconic" variant="outlined" icon="mdi-bookmark" nuxt to="/local"><v-icon color="iconic">mdi-bookmark</v-icon><v-tooltip class="sitesdesc" activator="parent" location="end">Local Domain</v-tooltip></v-btn>
    </div>    
</v-row>
