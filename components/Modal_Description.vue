<template>
  <v-col >
    <v-row >
    <div align="left" style="width: 100%; height: 5vh;padding-left:4px;padding-top:4px;background-color: white;" >  
      <span STYLE="font-size:14.0pt;font-family:arial; color: grey;">{{ xCurrDomainDesc }}</span>
    </div>    
  ...
  export default {
    ...
    xCurrDomainDesc() {
      ...
      console.log('** Start Desc-1')
      if(this.gdata == undefined){return res;}        
      console.log('** Start Desc-2')
      if(this.gdata.sCurrDomainDesc != undefined || this.gdata.sCurrDomainDesc != null) {res=this.gdata.sCurrDomainDesc;}
      console.log('** Start Desc-3:' + res)
      var sLocation = dload.getLocation()
      console.log('** Start Desc-4:' + res)
      if(sLocation == 'home'){res=this.gdata.sDomainDescription;}
      if(sLocation == 'local'){res='Local Desktop';}
      return res;
